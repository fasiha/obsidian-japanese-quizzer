...
6. **Recover multi-morpheme idioms and set phrases that `hits` is missing.** The `hits` array is built by a deterministic harness that anchors each candidate on an *exact-prefix* match. It cannot bridge a dropped particle or a contraction in the middle of an expression, so multi-word idioms are exactly what it silently omits. Use your knowledge of Japanese to decide *when* a run of adjacent content words is a set phrase worth recording — that judgment is the gate, and it keeps you from doing wasteful lookups. But once you judge that a set phrase is present, you **must** look it up: JMDict idioms are stored in **citation form** — particles present and verbs in dictionary form (訳が分からない, 気が付く, 仕方がない) — while the text is usually contracted (わけわかんない, きがついた), so you cannot guess the `wordId` or sense list and cannot annotate without fetching them from the database. Search with the multi-anchor mode, anchoring on two fragments you are confident about and letting it jump the particle and conjugation:

   ```bash
   node lookup.mjs 'わけ*わか'   # finds 訳が分からない (1538340) despite dropped が and contraction
   node lookup.mjs '訳*分'       # same, anchored on kanji
   ```

   `node lookup.mjs` search modes (always quote the asterisk): bare `食べる` = exact headword/reading; `'たべ*'` = prefix; `'*わかる'` = substring anywhere; `'X*Y'` = headword starting with X then later containing Y, in order. If an entry exists and fits, annotate it **and** its content-word components individually (per point 3). `hits` is reliable for single words, not for idioms with internal particles — do not assume it is complete for multi-word expressions.

   **If the lookup genuinely fails but you are confident the passage uses a set phrase, idiom, or colloquialism, annotate it anyway** using the `Not in JMDict` bare-string format (see 2d): `"Not in JMDict: {phrase as it appears in text} — {concise meaning in context}"`. JMDict can omit common colloquial usages, so a missing entry does not mean the phrase isn't worth recording — a plain-English bullet still helps the reader even though the app cannot turn it into a learnable card. This is a **last resort**, in this order of preference: a spanning entry already in `hits` → an `X*Y` lookup → a `Not in JMDict` bullet. Do not freestyle a definition for an ordinary single word that *is* in the dictionary just because MeCab mislabeled it — for those, find the correct entry (per point 5 and the homophone guidance).
